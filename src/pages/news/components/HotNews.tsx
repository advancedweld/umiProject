  ...
  return (
    ...
    <div className={styles.hotNewsWrap}>
      {isLoading ? (
        ...
        <>
          <img className={styles.img} src={newsDetail?.newsPicUrl} />
          <div className={styles.textContainer}>
            <div className={styles.title}>{newsDetail?.newsName}</div>
            <div className={styles.content}>{newsDetail?.newsDesc}</div>
            <div className={styles.btnCol}>
              <Button onClick={() => nav(`/news-detail/${newsDetail?.id}`)}>查看详情</Button>
              <span>{newsDetail?.newsTime}</span>
            </div>
          </div>
        </>
      )}
